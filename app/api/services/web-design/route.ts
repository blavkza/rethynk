...
export async function POST(req: Request) {
  try {
    ...
    const WebDesignService = await db.webDesignService.create({
      data: {
        designStyle,
        budgetRange,
        inspiration,
    ...
    return NextResponse.json(WebDesignService);
  } catch (error) {
...
export async function GET() {
  try {
    const WebDesignService = await db.webDesignService.findMany({
      include: {
        Milestones: true,
      },
    });
